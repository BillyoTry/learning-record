...
_STL_BEGIN_NAMESPACE
	...
	protected:
		...
		typedef simple_alloc<_Tp, _Alloc> _M_data_allocate;

		_Tp* _M_allocate(size_t __n) {
			return _M_data_allocate::allocate(__n);
		}

		void _M_deallocate(_Tp* __p, size_t __n) {
			_M_data_allocate::deallocate(__p, __n);
		}
	};
	...
	protected:
		void _M_insert_aux(iterator __position, const Tp_& __x);
	...
	public:
		...
		const_reference operator[](size_tpye __n) {
			return *(begin() + __n);
		}
		...
		//v.at(n)相当于v[n] 前者会检查是否越界，后者不会
		reference at(size_type __n) {
			_M_range_check(__n); //检查越界
			return (*this)[__n];   
		}
		...
		void reserve(size_type __n) {
			...
			if (capacity() < __n) {
				...
				destroy(_M_start, _M_finish);//清数据
				_M_deallocate(_M_start, _M_end_of_storage - _M_start);//清内存
				_M_start = __tmp;
		...
		iterator erase(iterator __position) {
			...
			if (__position + 1 != end()) {
				copy(__position + 1, _M_finish, __position);//指定位置的后一个元素到最后一个元素
			}
			--_M_finish;
			destroy(_M_finish);//删除一个元素后，全部往前copy后，删除最后一个元素。
			return __position;
		}

		iterator erase(iterator __first,iterator __last) {
			iterator __i = copy(__last, _M_finish, __first);  //越界
	...
	protected:
		template<class _ForwardIterator>
		iterator _M_allocate_and_copy(size_tpye __n, _ForwardIterator __first, _ForwardIterator __last) {
	...
	void vector<_Tp, _Alloc>::_M_assign_aux(_InputIter __first, _InputIter __last, input_iterator_tag) {
		...
		for (;__first != __last && __cur != end();++__cur, ++__first)
			*__cur = *__first;
		//如果size>__n(__last-__first)则擦除超过__n个的元素
		if (__first == __last)
			erase(__cur, end());
		...
		if (__n != 0) {
			...
			if (size_type(_M_end_of_storage - _M_finish) >= __n) {
				...
				if (__elem_after > __n) {
					...
					_M_finish += __n;						copy_backward(__position, __old_finish - __n, __old_finish);
		...
		if (__first != __last) {
			...
			if (size_type(_M_end_of_storage - _M_finish) >= __n) {
				...
				if (__elems_after > __n) {
					...
					advance(__mid, __elems_after); //__mid+=__elems_after
